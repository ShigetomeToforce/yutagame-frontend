...
interface GenreCSVFieldDiff {
  field: string;
  from: string;
  to: string;
}

interface GenreCSVRowInput {
  rowNumber: number;
  id?: number;
  name?: string;
  kana?: string;
...
interface GenreCSVOperation {
  rowNumber: number;
  action: "create" | "update" | "skip";
  id?: number;
  selectable: boolean;
  reason?: string;
  diffs: GenreCSVFieldDiff[];
  payload: GenreCSVRowInput;
}

interface GenreCSVPreviewResponse {
  operations: GenreCSVOperation[];
  creatable: number;
  updatable: number;
  skipped: number;
...
export default function GenreList(
  ...
  const preview = useSignal<GenreCSVPreviewResponse | null>(null);
  ...
  const handleExport = async () => {
    ...
    try {
      const response = await adminFetchRaw("/admin/genres/export", {
        method: "POST",
        body: JSON.stringify({
  ...
  const handlePreviewImport = async () => {
    ...
    try {
      ...
      const response = await adminFetch<GenreCSVPreviewResponse>(
        "/admin/genres/import/preview",
        {
          method: "POST",
          body: formData,
        },

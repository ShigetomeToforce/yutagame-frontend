...
export default function SearchRankingDashboard(props: Props) {
  ...
  const load = async () => {
    ...
    error.value = "";
    try {
      data.value = await fetchSearchRankingDashboard({
        field: props.field,
        period: period.value,
        date: date.value,
        month: month.value,
  ...
  return (
    <div class="space-y-4">
      <AnalyticsNav current={props.navKey} />
      ...
      {!loading.value && !error.value && data.value && (
        <section class="rounded-lg border border-gray-200 bg-white p-4">
          <h2 class="mb-3 text-lg font-bold text-gray-900">
            {props.title}ランキング ({formatTarget(
              data.value.period,
              data.value.target,
            )})
          </h2>
                ...
                <table class="min-w-full border-collapse">
                  <thead>
                    <tr class="border-y border-gray-200 bg-gray-50 text-xs font-semibold text-gray-600">
                      ...
                      <th class="px-3 py-2 text-left">{props.valueLabel}</th>

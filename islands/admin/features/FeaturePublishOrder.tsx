...
export default function FeaturePublishOrder() {
  const items = useSignal<FeatureItem[]>([]);
  ...
  const load = async () => {
    loading.value = true;
    try {
      items.value = await adminFetch<FeatureItem[]>(
        "/admin/features/published-order",
      );
    } catch (reason) {
  ...
  const reorder = async (targetId: number) => {
    ...
    saving.value = true;
    try {
      await adminFetch("/admin/features/order", {
        method: "POST",
  ...
  if (loading.value) {
    return <p class="text-sm text-slate-500">公開中の特集を読み込み中...</p>;
  }
  ...
  return (
    <section class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <header class="border-b border-slate-200 bg-slate-50 px-4 py-3">
        <h2 class="font-bold text-slate-800">公開中の特集の表示順</h2>
        <p class="mt-1 text-xs text-slate-500">
          ドラッグ＆ドロップで並び替えると、公開側の表示順（優先度）に反映されます。
        </p>
      </header>
      {error.value && (

...
export default function GameForm({ mode = "create", gameCode }: Props) {
    ...
    void (async () => {
      try {
        ...
        if (mode === "edit") {
          ...
          imageKey.value = game.imageKey ?? null;
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      await adminFetch(`/admin/games/${gameId.value}/image`, {
        method: "DELETE",
      });
      imageKey.value = null;
      selectedImageFile.value = null;
  ...
  const handleSubmit = async (event: Event) => {
    ...
    try {
      ...
      const savedGame = await adminFetch<GameRecord>(endpoint, {
        method,
            ...
            <div class="flex min-h-[240px] items-start justify-start overflow-hidden">
              <img
                src={previewImageUrl.value ??
                  buildImageUrl(imageKey.value, "games")}
                alt="ゲーム画像"
